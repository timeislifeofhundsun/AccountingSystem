...
-- --------------------	----------------------------------------------
delimiter $$
create procedure gpjy_resetYe(in p_ztbh int,in p_ywrq varchar(20))
begin
	-- 恢复交易费用
    ...
    -- 恢复证券清算款
 	UPDATE accountingsystem.t_ccyeb ye 
		SET ye.zqcb=ye.zqcb
		+ IFNULL((SELECT sum(t.zqqsk) FROM accountingsystem.t_qsb t 
		   WHERE t.ywlb = '1101' and t.ztbh=p_ztbh and t.rq=p_ywrq),0)  -- 股票买入
		- IFNULL((SELECT sum(t.zqqsk) FROM accountingsystem.t_qsb t 
		   WHERE t.ywlb = '1102' and t.ztbh=p_ztbh and t.rq=p_ywrq),0)  -- 股票卖出
	...
	UPDATE accountingsystem.t_ccyeb ye 
		SET ye.zqcb=ye.zqcb
        -IFNULL((select sum(t.cjsr) from accountingsystem.t_qsb t 
		WHERE t.ywlb = '1102'    and t.ztbh=p_ztbh and t.rq=p_ywrq),0)
